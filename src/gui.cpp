...
void GUI::render(double cpuUsage, const std::vector<double> &coreUsages,
  ...
  drawTab(" Intelligence ", Tab::Intelligence);
  drawTab("  Analytics  ", Tab::Analytics);
  ...
  case Tab::System:
    ...
    break;
  case Tab::Intelligence:
    renderIntelligenceTab(hpcReport);
    break;
  case Tab::Analytics:
    renderAnalyticsTab(sessionMonitor, hpcEngine);
    break;
  }
  ImGui::EndChild();
...
void GUI::renderSidebar(double cpuUsage, const MemoryInfo &memInfo,
  ...
  ImGui::SetCursorPosX(24);
  if (ImGui::Checkbox("Eco Mode", &gameMode)) {
    // Polling rate reduction logic
  }
...
void GUI::renderAnalyticsTab(GamingSessionMonitor& sessionMonitor, HpcEngine& hpcEngine) {
  ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(0.4f, 0.7f, 1.0f, 1.0f));
  ImGui::Text("ULTRA WORKSTATION PERFORMANCE HUB");
  ImGui::PopStyleColor();
  ImGui::Separator();
  ImGui::Spacing();
  ...
  if (!sessionMonitor.isActive()) {
    ...
    if (showSessionReport) {
      ...
      ImGui::TextColored(ImVec4(1.0f, 0.8f, 0.0f, 1.0f), "WORKSTATION INTELLIGENCE CORE");
      renderIntelligenceTab(hpcReportSession);

      ImGui::Spacing();
      if (ImGui::Button("EXPORT TXT", ImVec2(totalAvailWidth / 3.2f, 40))) {
          saveReportToTxt(hpcReportSession, metrics);
      }
      ImGui::SameLine();
      if (ImGui::Button("EXPORT JSON", ImVec2(totalAvailWidth / 3.2f, 40))) {
          saveReportToJson(hpcReportSession, metrics);
      }
      ImGui::SameLine();
      if (ImGui::Button("EXPORT CSV", ImVec2(totalAvailWidth / 3.2f, 40))) {
          saveReportToCsv(hpcReportSession, metrics);
      }
    }
  } else {
    // ACTIVE PANEL
    ...
    {
      ImGui::SetCursorPos(ImVec2(20, 20));
      ImGui::TextColored(ImVec4(1.0f, 0.2f, 0.2f, 1.0f), "[ ANALYTICS ENGINE SCANNING ]");
...
void GUI::saveReportToTxt(const HpcReport& report, const SessionMetrics& metrics) {
    std::string filename = "Workstation_Performance_Report.txt";
    std::ofstream out(filename);
    
    if (out.is_open()) {
        out << "====================================================\n";
        out << "   WORKSTATION PERFORMANCE & ANALYTICS REPORT\n";
        out << "====================================================\n\n";
        ...
        out << "\nGenerated by System Intelligence Engine\n";
        out << "====================================================\n";
        out.close();
    }
}

void GUI::saveReportToJson(const HpcReport& report, const SessionMetrics& metrics) {
    std::string filename = "Workstation_Session_Data.json";
    std::ofstream out(filename);
    if (out.is_open()) {
        out << "{\n";
        out << "  \"session_info\": {\n";
        out << "    \"duration_seconds\": " << metrics.durationSeconds << ",\n";
        out << "    \"avg_fps\": " << metrics.avgFps << ",\n";
        out << "    \"min_fps\": " << metrics.minFps << ",\n";
        out << "    \"max_fps\": " << metrics.maxFps << ",\n";
        out << "    \"avg_ping_ms\": " << metrics.avgPing << ",\n";
        out << "    \"max_ping_ms\": " << metrics.maxPing << ",\n";
        out << "    \"avg_cpu_load\": " << metrics.avgCpuUsage << ",\n";
        out << "    \"avg_gpu_load\": " << metrics.avgGpuUsage << ",\n";
        out << "    \"max_gpu_temp\": " << metrics.maxGpuTemp << "\n";
        out << "  },\n";
        ...
        for (size_t i = 0; i < report.insights.size(); ++i) {
            out << "      {\n";
            out << "        \"title\": \"" << report.insights[i].title << "\",\n";
            out << "        \"level\": " << (int)report.insights[i].level << "\n";
            out << "      }" << (i == report.insights.size() - 1 ? "" : ",") << "\n";
        }
        out << "    ]\n";
        out << "  },\n";

        // Export history for data scientists
        auto exportVector = [&](const char* name, const std::vector<float>& vec, bool isLast) {
            out << "  \"" << name << "\": [";
        ...
        exportVector("fps_timeline", metrics.fpsHistory, false);
        exportVector("latency_timeline", metrics.pingHistory, false);
        exportVector("cpu_util_timeline", metrics.cpuHistory, false);
        exportVector("gpu_util_timeline", metrics.gpuHistory, true);
...
void GUI::saveReportToCsv(const HpcReport& report, const SessionMetrics& metrics) {
    std::string filename = "Workstation_Telemetry_Timeline.csv";
    std::ofstream out(filename);
    if (out.is_open()) {
        out << "Timestamp(s),FPS,Ping(ms),CPU_Load(%),GPU_Load(%)\n";
        
        size_t maxSize = metrics.fpsHistory.size();
        maxSize = (std::max)(maxSize, metrics.pingHistory.size());
        maxSize = (std::max)(maxSize, metrics.cpuHistory.size());
        maxSize = (std::max)(maxSize, metrics.gpuHistory.size());

        for (size_t i = 0; i < maxSize; ++i) {
            out << i << ",";
            out << (i < metrics.fpsHistory.size() ? metrics.fpsHistory[i] : 0.0f) << ",";
            out << (i < metrics.pingHistory.size() ? metrics.pingHistory[i] : 0.0f) << ",";
            out << (i < metrics.cpuHistory.size() ? metrics.cpuHistory[i] : 0.0f) << ",";
            out << (i < metrics.gpuHistory.size() ? metrics.gpuHistory[i] : 0.0f) << "\n";
        }
        out.close();
    }
}
